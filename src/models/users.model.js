...
export const updateUserById = async (user_id, userData) => {
  const { username, rut, birth_date, email, phone, password, role, status } =
    userData;
...
export const changeUserStatus = async (user_id, status) => {
  const query = format(
    "UPDATE users SET status = %L WHERE user_id = %s RETURNING *",
    status,
    user_id
  );
  try {
...
export const changeRole = async (user_id, role) => {
  const query = format(
    "UPDATE users SET role = %L WHERE user_id = %s RETURNING *",
    role,
    user_id
  );
  try {
